...
import { getPlanById, getUserById, updateSale } from "../../services/api";

const EditModal = ({ isOpen, onCancel, onSave, sale }) => {
  ...
  useEffect(() => {
    if (sale) {
      setFormData({
        ...
        special_requirement: sale.special_requirement || "",
        additional_price: sale.additional_price || 0,
      });
    }
  }, [sale]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setErrorModal("It seems that you entered an invalid id. Please try again.");
    }
  };
